...
class data:
    def __init__(self, scidir, scifiles, caldir, calfiles):
        """
        Parameters
        ----------
        scidir: str
        ...
        for i in range(len(self.scifiles)):
            inst_list, data_list = inst.open(idir=scidir, fitsfile=self.scifiles[i], verbose=False)
        ...
        for i in range(len(self.calfiles)):
            inst_list, data_list = inst.open(idir=caldir, fitsfile=self.calfiles[i], verbose=False)
    ...
    def calibrate(self, odir, K_klip=4):
        """
        Parameters
        ----------
        odir: str
    ...
    def decompose(self, K_klip=4):
        ...
        for i in range(len(self.observables)):
            ...
            self.P[self.observables[i]] = np.divide(
                self.P[self.observables[i]].T, np.linalg.norm(self.P[self.observables[i]], axis=1)
            ).T
    ...
    def project(self, odir):
        ...
        for i in range(Nscifiles):
            ...
            if 'KP-DATA' in hdul:
                ...
                for j in range(len(self.observables)):
                    if self.observables[j] == 'kp':
                        ...
                        if len(hdul['KP-DATA'].data.shape) == 1:
                            ...
                            temp = np.zeros(
                                (
                                    hdul['KP-DATA'].data.shape[0],
                                    hdul['KP-DATA'].data.shape[1],
                                    self.P[self.observables[j]].shape[0],
                                )
                            )
                            for k in range(hdul['KP-DATA'].data.shape[0]):
                        ...
                        if len(hdul['KP-SIGM'].data.shape) == 1:
                            ...
                            try:
                                ...
                                hdul['KP-SIGM'].data = (
                                    self.P[self.observables[j]]
                                    .dot(hdul['KP-SIGM'].data)
                                    .dot(self.P[self.observables[j]].T)
                                )
                            ...
                            try:
                                ...
                                temp = np.zeros(
                                    (
                                        hdul['KP-SIGM'].data.shape[0],
                                        hdul['KP-SIGM'].data.shape[1],
                                        self.P[self.observables[j]].shape[0],
                                    )
                                )
                                for k in range(hdul['KP-SIGM'].data.shape[0]):
                        ...
                        try:
                            if len(hdul['EKP-SIGM'].data.shape) == 1:
                                ...
                                temp = np.zeros(
                                    (
                                        hdul['EKP-SIGM'].data.shape[0],
                                        hdul['EKP-SIGM'].data.shape[1],
                                        self.P[self.observables[j]].shape[0],
                                    )
                                )
                                for k in range(hdul['EKP-SIGM'].data.shape[0]):
                        ...
                        try:
                            if len(hdul['KP-COV'].data.shape) == 2:
                                hdul['KP-COV'].data = (
                                    self.P[self.observables[j]]
                                    .dot(hdul['KP-COV'].data)
                                    .dot(self.P[self.observables[j]].T)
                                )
                            elif len(hdul['KP-COV'].data.shape) == 3:
                                temp = np.zeros(
                                    (
                                        hdul['KP-COV'].data.shape[0],
                                        self.P[self.observables[j]].shape[0],
                                        self.P[self.observables[j]].shape[0],
                                    )
                                )
                        ...
                        try:
                            if len(hdul['EKP-COV'].data.shape) == 2:
                                hdul['EKP-COV'].data = (
                                    self.P[self.observables[j]]
                                    .dot(hdul['EKP-COV'].data)
                                    .dot(self.P[self.observables[j]].T)
                                )
                            elif len(hdul['EKP-COV'].data.shape) == 3:
                                temp = np.zeros(
                                    (
                                        hdul['EKP-COV'].data.shape[0],
                                        self.P[self.observables[j]].shape[0],
                                        self.P[self.observables[j]].shape[0],
                                    )
                                )
                ...
                sys.stdout.write(
                    '\r   File %.0f of %.0f: OIFITS file -- !!! KL calibration for V2 is experimental !!!'
                    % (i + 1, Nscifiles)
                )
                sys.stdout.flush()
                    ...
                    if self.observables[j] == 'v2':
                        ...
                        if hdul['OI_VIS2'].data['VIS2DATA'].ndim == 1:
                            ...
                            if 'V2COV' in hdul:
                                hdul['V2COV'].data = (
                                    self.P[self.observables[j]]
                                    .dot(hdul['V2COV'].data[0])
                                    .dot(self.P[self.observables[j]].T)[np.newaxis, :]
                                )
                        else:
                            hdu1 = pyfits.ImageHDU(
                                np.mean(
                                    self.P[self.observables[j]].dot(hdul['OI_VIS2'].data['VIS2DATA']),
                                    axis=1,
                                    keepdims=True,
                                )
                            )
                        ...
                        if hdul['OI_T3'].data['T3PHI'].ndim == 1:
                            ...
                            if 'CPCOV' in hdul:
                                hdul['CPCOV'].data = (
                                    self.P[self.observables[j]]
                                    .dot(hdul['CPCOV'].data[0])
                                    .dot(self.P[self.observables[j]].T)[np.newaxis, :]
                                )
                        else:
                            hdu1 = pyfits.ImageHDU(
                                np.mean(
                                    self.P[self.observables[j]].dot(hdul['OI_T3'].data['T3PHI']), axis=1, keepdims=True
                                )
                            )
                            hdu1.header['EXTNAME'] = 'T3PHI'
                            errs = []
    ...
    def calibrate_classical(self, odir, mean=False):
        """
        Parameters
        ----------
        odir: str
            ...
            if 'KP-DATA' in hdul:
                ...
                for j in range(len(self.observables)):
                    if self.observables[j] == 'kp':
                        ...
                        hdul['KP-SIGM'].data = np.sqrt(
                            hdul['KP-SIGM'].data.copy() ** 2 + np.mean(kp_dkp_cal, axis=0) ** 2 / kp_dkp_cal.shape[0]
                        )
                        hdul['KP-COV'].data = (
                        ...
                        pyfits.ColDefs(
                            [
                                pyfits.Column(
                                    ...
                                    format='1E',
                                    unit='METERS',
                                    array=np.array([np.mean(hdul['OI_WAVELENGTH'].data['EFF_WAVE'])]),
                                ),
                                pyfits.Column(
                                    name='EFF_BAND',
                                    format='1E',
                                    unit='METERS',
                                    array=np.array([np.mean(hdul['OI_WAVELENGTH'].data['EFF_BAND'])]),
                                ),
                            ]
                        )
                    )
                ...
                for j in range(len(self.observables)):
                    if self.observables[j] == 'v2':
                        ...
                        if not mean:
                            hdul['OI_VIS2'].data['VIS2DATA'] = np.true_divide(
                                hdul['OI_VIS2'].data['VIS2DATA'].copy(), np.mean(oi_v2_cal, axis=0)
                            )
                            hdul['OI_VIS2'].data['VIS2ERR'] = np.sqrt(
                                hdul['OI_VIS2'].data['VIS2ERR'].copy() ** 2
                                + (np.mean(oi_dv2_cal, axis=0) / np.sqrt(oi_dv2_cal.shape[0])) ** 2
                            )
                        else:
                            ...
                            v2d = np.true_divide(
                                np.mean(hdul['OI_VIS2'].data['VIS2DATA'], axis=1), np.mean(oi_v2_cal, axis=(0, 2))
                            )
                            ...
                            hdu_v2 = pyfits.BinTableHDU.from_columns(
                                pyfits.ColDefs(
                                    [
                                        pyfits.Column(
                                            name='TARGET_ID', format='1I', array=hdul['OI_VIS2'].data['TARGET_ID']
                                        ),
                                        pyfits.Column(
                                            name='TIME', format='1D', unit='SECONDS', array=hdul['OI_VIS2'].data['TIME']
                                        ),
                                        pyfits.Column(
                                            name='MJD', format='1D', unit='DAY', array=hdul['OI_VIS2'].data['MJD']
                                        ),
                                        pyfits.Column(
                                            name='INT_TIME',
                                            format='1D',
                                            unit='SECONDS',
                                            array=hdul['OI_VIS2'].data['INT_TIME'],
                                        ),
                                        ...
                                        pyfits.Column(
                                            ...
                                            format='1D',
                                            unit='METERS',
                                            array=hdul['OI_VIS2'].data['UCOORD'],
                                        ),
                                        pyfits.Column(
                                            name='VCOORD',
                                            format='1D',
                                            unit='METERS',
                                            array=hdul['OI_VIS2'].data['VCOORD'],
                                        ),
                                        pyfits.Column(
                                            name='STA_INDEX', format='2I', array=hdul['OI_VIS2'].data['STA_INDEX']
                                        ),
                        ...
                        if not mean:
                            hdul['OI_T3'].data['T3PHI'] = hdul['OI_T3'].data['T3PHI'].copy() - np.mean(
                                oi_cp_cal, axis=0
                            )
                            hdul['OI_T3'].data['T3PHIERR'] = np.sqrt(
                                hdul['OI_T3'].data['T3PHIERR'].copy() ** 2
                                + (np.mean(oi_dcp_cal, axis=0) / np.sqrt(oi_dcp_cal.shape[0])) ** 2
                            )
                        else:
                            ...
                            hdu_cp = pyfits.BinTableHDU.from_columns(
                                pyfits.ColDefs(
                                    [
                                        pyfits.Column(
                                            name='TARGET_ID', format='1I', array=hdul['OI_T3'].data['TARGET_ID']
                                        ),
                                        pyfits.Column(
                                            name='TIME', format='1D', unit='SECONDS', array=hdul['OI_T3'].data['TIME']
                                        ),
                                        pyfits.Column(
                                            name='MJD', format='1D', unit='DAY', array=hdul['OI_T3'].data['MJD']
                                        ),
                                        pyfits.Column(
                                            name='INT_TIME',
                                            format='1D',
                                            unit='SECONDS',
                                            array=hdul['OI_T3'].data['INT_TIME'],
                                        ),
                                        ...
                                        pyfits.Column(
                                            ...
                                            format='1D',
                                            unit='METERS',
                                            array=hdul['OI_T3'].data['U1COORD'],
                                        ),
                                        pyfits.Column(
                                            name='V1COORD',
                                            format='1D',
                                            unit='METERS',
                                            array=hdul['OI_T3'].data['V1COORD'],
                                        ),
                                        pyfits.Column(
                                            name='U2COORD',
                                            format='1D',
                                            unit='METERS',
                                            array=hdul['OI_T3'].data['U2COORD'],
                                        ),
                                        pyfits.Column(
                                            name='V2COORD',
                                            format='1D',
                                            unit='METERS',
                                            array=hdul['OI_T3'].data['V2COORD'],
                                        ),
                                        pyfits.Column(
                                            name='STA_INDEX', format='3I', array=hdul['OI_T3'].data['STA_INDEX']
                                        ),
                                        pyfits.Column(name='FLAG', format='%iL' % owl, array=flag),
                                    ]
                                )
